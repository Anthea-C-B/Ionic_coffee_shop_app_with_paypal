...
export class CheckoutPage implements OnInit{
  ...
  purchase(){
    if(this.discountUsed){
      let rewId = this.discount.rewardId;
      let cal = this.rewardsList.map(b => b.rewardId).indexOf(rewId);
      if(cal > -1){
        this.rewardsList.splice(cal,1);
      }
  ...
  payCart(amt){
    this.payPal.init({
      ...
      PayPalEnvironmentSandbox:'AekXdKnmMNud3NnFPzn4-G82sfyuQF4HURNVH5Jvv2x9JemXqp6FTX8HY-BFR_CqCed2C6QvJxB4M39I'
    }).then(() =>{
      this.payPal.prepareToRender('PayPalEnvironmentSandbox',new PayPalConfiguration({
      })).then(()=>{
        let payment = new PayPalPayment(amt, 'GBP','Description','sale');
